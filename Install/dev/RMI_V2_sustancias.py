...
class unidGeo(object):
    def __init__(self):
        ...
        self.domains = domains[geologia_tex]
        self.msg = Messages()
        self.information = []
        self.dataset = ds_insumos
        self.feature = geologia
        self.campo = [fd_grado, fd_valor, fd_condicion]
            ...
            if desc.datatype == u'Workspace':
                if arcpy.Exists(os.path.join(self.ws, self.dataset, self.feature)):
                    if len(self.information) > 0:
                        ...
                        del cursorUC
                        campos = {fd_codi: self.codi, self.campo[0]: self.grado, self.campo[1]: self.value, self.campo[2]: self.condition}
